...
for D in [2,4,6,8,10,12,14,16,18]:
    ...
    X, Y = np.linspace(-2,3,res, dtype="float32"), np.linspace(-7,-2,res, dtype="float32")
    ...
    dX = X[1]-X[0]
    dY = Y[1]-Y[0]
    visualise_array_2d(X-dX/2, Y+dY/+dY/2, np.log10(scores), ax=ax)
    plt.plot(X[best_ind[0]], Y[best_ind[1]], 'ro', markersize=15)
    ...
    with h5.File('results/param_search/'+file_name+".h5", 'w') as f:
        ...
        f.create_dataset("all_scores", data = all_scores)
