...
def load_data(name, data_array=None, data_array_name=None, D = None, noise_std=0.0, seed=1, **kwargs):
    ...
    if name == 'mnist':
        ...
        data = datasets.POWER(noise_std=noise_std, seed=seed, **kwargs)
        data_name = name

    elif name == 'gas':
        data = datasets.GAS(noise_std=noise_std, seed=seed, **kwargs)
        data_name = name
        ...
        data = datasets.REDWINE(noise_std=noise_std, seed=seed, **kwargs)
        data_name = name
    ...
    elif name in ['multiring','funnel', 'ring', 'spiral', 'banana', 'cosine', 'grid', 'uniform']:
        assert D is not None
        data_name = name
        name = name.title()
        kwargs["whiten"]=False
        kwargs["itanh"]=False
        data = datasets.TOY(name, D, noise_std=noise_std, seed=seed, **kwargs)
    else:
...
def train(model, a, maxepochs=None):
