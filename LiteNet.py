...
class DeepGaussianKernel(Kernel):
    ...
    def __init__(self, Ds, init_std = 1.0, trainable=True):
        self.Ds = Ds
        with tf.name_scope("GeneralGaussianKernel"):

            W = tf.Variable(np.array([np.eye(Ds[-1])]*self.Ds[0]), dtype=FDTYPE)
            self.W = tf.matmul(W,W,transpose_a=True)
    '''
    def Sigma(self, X):
        
        for i in range(len(self.Ds)-1):
            W, b = self.layers[i]
            X = tf.matmul(X, W, transpose_b=True) + b
            if i != len(self.Ds)-2:
                X = tf.nn.softplus(X)
        D = int(np.sqrt(self.Ds[-1]))
        X = tf.reshape(X, [-1, D,D])
    ...
    def quad(self, X, Y):
        ...
        XS = tf.matmul(X[:,None,:], S)
        quad1 = tf.squeeze(XS, 1)
        quad1 = tf.reduce_sum(quad1*X, -1)[:,None]
        
        quad2 = tf.reduce_sum(XS*Y[None], -1)

        SY = tf.einsum("bj,aij->abi", Y, S)
        quad3 = tf.reduce_sum(SY*Y[None], -1)
            
        q = quad1 - 2*quad2 + quad3

        return q, S

    def get_gram_matrix(self, X, Y):

        X, Y, single = self.check_input_dim(X, Y)
        
        quad, S = self.quad(X, Y)
        gram = tf.exp(-0.5*quad)
        if single:
            gram = gram[0,0]
        return gram
    ...
    def get_grad(self, X, Y):
        ...
        D = tf.einsum('aij,aj->ai', S, X)[:,None,:] - tf.einsum('aij,bj->abi', S, Y)
    ...
    def get_sec_grad(self, X, Y):
        ...
        gram = gram
        D2 = tf.square(D)
        K2 = gram * (D2 - tf.expand_dims(tf.matrix_diag_part(S),1))
